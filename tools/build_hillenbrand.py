...
def fetch_zip() -> bytes:
    req = urllib.request.Request(SOURCE_ZIP, headers={"User-Agent": "FormantCanvas-Hillenbrand-builder/1.1"})
...
def main():
    ...
    source_vowels = Counter(r["vowel"] for r in rows)
    print("source vowel codes:", dict(sorted(source_vowels.items())))
    unknown = sorted(set(source_vowels) - set(VOWEL_LABELS))
    missing = sorted(set(VOWEL_LABELS) - set(source_vowels))
    if unknown or missing:
        raise RuntimeError(f"Vowel-code mismatch: unknown={unknown}, missing={missing}")
    ...
    payload = {
        ...
        "source": {
            ...
            "source_vowel_counts": dict(sorted(source_vowels.items())),
        },
